...
import net.astrorbits.lib.codec.builder.CodecBuilder
import net.astrorbits.lib.codec.builder.CodecWithGetter
import net.astrorbits.lib.text.TextHelper
import net.astrorbits.lib.text.TextHelper.toJsonString
...
interface Codec<T> {
    ...
    fun encode(obj: T, encoder: ObjectEncoder): ObjectEncoder {
        encoder.put(obj, this)
        return encoder
    }

    fun decode(decoder: ObjectDecoder): T {
        return decoder.get(this)
    }
    ...
    fun <O> forGetter(getter: Function<O, T>): CodecWithGetter<O, T> = CodecWithGetter(this) { getter.apply(it) }
    ...
    companion object {
        ...
        val TEXT: Codec<Component> = STRING.xmap(TextHelper::toComponent) { it.toJsonString() }
        val COMPONENT: Codec<Component> = TEXT
        ...
        fun <E : Enum<E>> ofEnum(enumClass: Class<E>): SizedCodec<E> {
            return object : SizedCodec<E>(4) {
                override fun encodeSized(obj: E): ByteArray = ObjectEncoder(4).putEnum(obj).getData()
                override fun decodeSized(data: ByteArray): E = ObjectDecoder(data).getEnum(enumClass)
            }
        }
