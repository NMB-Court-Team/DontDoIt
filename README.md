...
可以根据配置里的注释进行增删，增加词条需要注意符合YAML格式，如`:`，`-`后需要加空格，缩进一致等。

注意事项：

- *有小部分词条是由插件动态生成的，可以在设置里进行开关，但不能增加。
...
You can add or remove criteria based on the configuration comments. When adding criteria, please ensure they conform to the YAML format, such as adding a space after : and -, maintaining consistent indentation, etc.


Note:
- *A small number of criteria are dynamically generated by plugin. These can be toggled on/off in the settings but cannot be added.
